...
def _is_git_try(remote):
    """Whether pushing to `remote` lands in a Git try repository.

    Until further notice, assume a git try is a non hg try: canonical firefox
    try on git is not yet solved, enterprise try git tree cannot be hardcoded
    either.
    """
