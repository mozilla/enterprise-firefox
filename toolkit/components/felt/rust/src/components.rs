...
impl FeltXPCOM {
    ...
    xpcom_method!(set_close_lock_intent => SetCloseLockIntent(lock: bool, reason: *const nsACString));
    fn set_close_lock_intent(
        &self,
        lock: bool,
        reason: &nsACString,
    ) -> Result<(), nserror::nsresult> {
        let reason = reason.to_string();
        trace!("FeltXPCOM::SetCloseLockIntent({}, {})", lock, reason);
        crate::CLOSE_LOCK_INTENT.store(lock, Ordering::Relaxed);
        match crate::CLOSE_LOCK_REASON.lock() {
            Ok(mut guard) => {
                *guard = (!reason.is_empty()).then_some(reason);
                Ok(())
            }
            Err(_) => {
                trace!("setCloseLockIntent(): reason lock poisoned");
                ...
                if let Some(rx) = rx_clone {
                    loop {
                        match rx.recv() {
                            ...
                            Ok(FeltMessage::Exiting(with_lock, reason)) => {
                                trace!("FeltServerThread::felt_server::ipc_loop(): Exiting, with_lock={}, reason={:?}", with_lock, reason);
