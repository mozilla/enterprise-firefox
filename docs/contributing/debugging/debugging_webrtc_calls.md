...
[link](https://bugzilla.mozilla.org/enter_bug.cgi?product=Core&component=WebRTC).
...
(share-your-aboutwebrtc-contents)=

## Adding <about:webrtc> RTCPeerConnection Stats

01. Open <about:webrtc>.
02. Expand the RTCPeerConnection section.
03. Locate and expand the subsection RTCPeerConnection that one wishes to copy
    stats from.
04. Press the {code}`Copy Stats Report`
05. In the Bugzilla bug, press the {code}`Attach New File` button.
06. Click inside the large text box labeled {code}`File`, and paste the copied
    stats report.
07. Add a descriptive label to the the {code}`Description:`, e.g.
    "PeerConnection statistics sample taken during period of frame loss".
08. In the drop down box, next to the {code}`select from list` radio option,
    select "JSON source (application/json)".
09. If needed, add a descriptive comment to the {code}`comment` field.
10. Press the {code}`Submit` button.
...
1. While your call is still ongoing, open an additional tab and visit
   <about:webrtc>.
2. Click "Clear History" to clear the stats from other recent calls
...
noticeable delay. See the [Delay Calculation](#delay-calculation)
...
1. Visit <https://profiler.firefox.com/> and enable the Profiler toolbar
   button.
2. Click the toolbar button down arrow and select 'Media' in the
   Settings drop down.
...
7. When a new Profile tab opens, click the upload profile button on the
   upper right.
...
1. Visit <https://profiler.firefox.com/> and enable the Profiler toolbar
   button.
2. In a new tab, visit <about:webrtc>. Click the 'Enable WebRTC Log
   Preset' button, which will open a tab for <about:logging> with
   pre-populated information.
3. In <about:logging>, click the "Start Logging" button. (You are now
   recording a profile, the profiler toolbar toggle button should be
   selected automatically.)
4. Open a new tab for testing and view the media you are having an issue
   with. (After reproducing, DO NOT close this test tab yet.)
5. Switch to the <about:logging> tab, click 'Stop logging', and then close
   the test tab.
6. Wait approximately 10 - 20 seconds for a new tab to automatically
   open containing the generated performance profile.
7. Within the upper-right side of the profiler tab click the 'upload
...
8. Share the URL of the profile for analysis with the engineer you are
   working with.
...
:::{warning}
There is a [blog
post](https://blog.mozilla.org/webrtc/debugging-encrypted-rtp-is-more-fun-than-it-used-to-be/)
...
then be explored in [Wireshark](https://www.wireshark.org/). The logs
...
see the MDN Page: [Codecs Used by WebRTC](https://developer.mozilla.org/en-US/docs/Web/Media/Formats/WebRTC_codecs).
...
repository](https://github.com/web-platform-tests/wpt), and likewise
our changes are synced from our [in-tree
copy](https://searchfox.org/mozilla-central/search?q=&path=testing%2Fweb-platform%2Ftests%2Fwebrtc&case=false&regexp=false)
...
[webrtc-pc](https://www.w3.org/TR/webrtc/), and
[webrtc-stats](https://www.w3.org/TR/webrtc-stats/). The number of IETF
standards that are incorporated into WebRTC are too numerous to list here. One
can find these standards in the
[Normative References](https://www.w3.org/TR/webrtc/#normative-references)
...
negotiation](https://developer.mozilla.org/en-US/docs/Web/API/WebRTC_API/Perfect_negotiation)
on MDN and RFC 9429
[§4.1.10.2](https://www.rfc-editor.org/rfc/rfc9429#section-4.1.10.2)
and [§5.7](https://www.rfc-editor.org/rfc/rfc9429#section-5.7).
...
on MDN and RFC 9429
[§3.2](https://www.rfc-editor.org/rfc/rfc9429#section-3.2).
...
about
[pendingLocalDescription](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/pendingLocalDescription)
and
[pendingRemoteDescription](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/pendingRemoteDescription)
on MDN and RFC 9429
[§4.1.14](https://www.rfc-editor.org/rfc/rfc9429#section-4.1.14) and
[§4.1.16](https://www.rfc-editor.org/rfc/rfc9429#section-4.1.16).
...
[currentLocalDescription](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/currentLocalDescription)
and
[currentRemoteDescription](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/currentRemoteDescription)
on MDN and RFC 9429
[§4.1.13](https://www.rfc-editor.org/rfc/rfc9429#section-4.1.13) and
[§4.1.15](https://www.rfc-editor.org/rfc/rfc9429#section-4.1.15).
...
more about
[transports](https://developer.mozilla.org/en-US/docs/Web/API/RTCDtlsTransport#allocation_of_dtls_transports)
and [bundle
configuration](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/RTCPeerConnection#bundlepolicy)
on MDN.
...
on MDN and [trickle
detection](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/canTrickleIceCandidates)
on in the JSEP spec.
...
fingerprints](https://developer.mozilla.org/en-US/docs/Web/API/RTCCertificate/getFingerprints)
on MDN and RFC 8122
[§5](https://www.rfc-editor.org/rfc/rfc8122#section-5) and RFC 4572
[§1](https://www.rfc-editor.org/rfc/rfc4572#section-1).
...
[§3](https://www.rfc-editor.org/rfc/rfc8445#section-3).
...
on MDN and RFC 8445
[§2](https://www.rfc-editor.org/rfc/rfc8445#section-2).
...
and [candidate pair
stats](https://developer.mozilla.org/en-US/docs/Web/API/RTCIceCandidatePairStats)
on MDN and RFC 8445
[§2](https://www.rfc-editor.org/rfc/rfc8445#section-2).
...
parameters](https://www.iana.org/assignments/rtp-parameters) via the
...
[§1](https://datatracker.ietf.org/doc/html/rfc5109#section-1).
...
[Opus](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#opus)
on MDN.

**VP8** An open video codec developed by Google, mandatory for WebRTC
implementations. Learn more about
[VP8](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#vp8)
on MDN.
...
[AV1](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#av1)
on MDN.

**H.264/AVC (Advanced Video Coding)** A widely-used video codec,
mandatory for WebRTC implementations. Learn more about
[AVC/H.264](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#avc_h.264)
on MDN.
...
codecs](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/Video_codecs)
on MDN.

**G.722** A wideband audio codec operating at 64 kbit/s. Learn more
about
[G.722](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/Audio_codecs#g.722_64_kbps_7_khz_audio_coding)
on MDN.

**PCMU/PCMA** G.711 audio codecs (μ-law and A-law) offering basic audio
quality, mandatory for WebRTC. Learn more about
[G.711](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#g.711)
on MDN.
...
level
indication](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpSender/getParameters#headerextensions)
on MDN and RFC 6464
[§1](https://www.rfc-editor.org/rfc/rfc6464#section-1).
...
[§1](https://www.rfc-editor.org/rfc/rfc6465#section-1).
...
8285 [§1](https://www.rfc-editor.org/rfc/rfc8285#section-1).
...
[§2](https://datatracker.ietf.org/doc/html/draft-holmer-rmcat-transport-wide-cc-extensions#section-2).
...
[§1](https://www.rfc-editor.org/rfc/rfc9143#section-1).
...
RFC 8852 [§1](https://www.rfc-editor.org/rfc/rfc8852#section-1).
...
[§1](https://www.ietf.org/archive/id/draft-ietf-avtcore-abs-capture-time-00.html#section-1).
...
[§1](https://www.rfc-editor.org/rfc/rfc5450#section-1).
...
specification](https://webrtc.googlesource.com/src/+/refs/heads/main/docs/native-code/rtp-hdrext/abs-send-time/).
...
specification](https://webrtc.googlesource.com/src/+/refs/heads/main/docs/native-code/rtp-hdrext/video-content-type/).
...
specification](https://webrtc.googlesource.com/src/+/refs/heads/main/docs/native-code/rtp-hdrext/video-timing/).
...
specification](https://webrtc.googlesource.com/src/+/refs/heads/main/docs/native-code/rtp-hdrext/playout-delay/).
...
specification](https://webrtc.googlesource.com/src/+/refs/heads/main/docs/native-code/rtp-hdrext/video-frame-tracking-id/).
...
RFC 7941 [§1](https://www.rfc-editor.org/rfc/rfc7941#section-1).
...
[scalability modes](https://developer.mozilla.org/en-US/docs/Web/API/RTCOutboundRtpStreamStats/scalabilityMode)
on MDN and in the W3C [Scalable Video Coding (SVC) Extension](https://www.w3.org/TR/webrtc-svc/) specification.
...
[SDP](https://developer.mozilla.org/en-US/docs/Glossary/SDP) on MDN
and RFC 8866
[§5.14](https://www.rfc-editor.org/rfc/rfc8866#section-5.14).
...
about
[maxBitrate](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpSender/setParameters#maxbitrate)
on MDN and RFC 3890
[§6.2.2](https://datatracker.ietf.org/doc/html/rfc3890#section-6.2.2).
...
profile-level-id](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#profile-level-id)
on MDN and RFC 6184
[§8.1](https://www.rfc-editor.org/rfc/rfc6184#section-8.1).
...
[max-fs](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#max-fs)
on MDN and RFC 6184
[§8.1](https://www.rfc-editor.org/rfc/rfc6184#section-8.1).
...
[maxFramerate](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpSender/setParameters#maxframerate)
on MDN and RFC 6184
[§8.1](https://www.rfc-editor.org/rfc/rfc6184#section-8.1).
...
[max-mbps](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#max-mbps)
on MDN and RFC 6184
[§8.1](https://www.rfc-editor.org/rfc/rfc6184#section-8.1).
...
[§6.1](https://www.rfc-editor.org/rfc/rfc7741#section-6.1).
...
[§6](https://www.rfc-editor.org/rfc/rfc9628#section-6).
...
specification](https://aomediacodec.github.io/av1-rtp-spec/#sdp-parameters).
...
[§7.1](https://www.rfc-editor.org/rfc/rfc7798#section-7.1).
...
RFC 7798 [§7.1](https://www.rfc-editor.org/rfc/rfc7798#section-7.1).
...
in RFC 5104
[§3.5.4](https://datatracker.ietf.org/doc/html/rfc5104#section-3.5.4).
...
[§2](https://datatracker.ietf.org/doc/html/draft-alvestrand-rmcat-remb#section-2).
...
[§1](https://datatracker.ietf.org/doc/html/rfc8627#section-1).
...
codecs](https://developer.mozilla.org/en-US/docs/Web/Media/Guides/Formats/WebRTC_codecs#supported_audio_codecs)
on MDN and RFC 7587
[§6.1](https://www.rfc-editor.org/rfc/rfc7587#section-6.1).
...
[§6.4](https://datatracker.ietf.org/doc/html/rfc8866#section-6.4).

**Maxptime (Maximum Packet Time)** The maximum acceptable ptime value.
Learn more about maxptime in RFC 8866
[§6.4](https://www.rfc-editor.org/rfc/rfc8866#section-6.4).
...
estimation](https://developer.mozilla.org/en-US/docs/Web/API/RTCIceCandidatePairStats/availableOutgoingBitrate)
on MDN.
...
on MDN and RFC 9260
[§1](https://datatracker.ietf.org/doc/html/rfc9260#section-1).

**Data Channel** A bidirectional data transport for arbitrary
application data over WebRTC. Learn more about
[RTCDataChannel](https://developer.mozilla.org/en-US/docs/Web/API/RTCDataChannel)
on MDN and RFC 8831
[§6.4](https://datatracker.ietf.org/doc/html/rfc8831#name-data-channel-definition).
...
and
[addTransceiver](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/addTransceiver)
on MDN.
...
[setCodecPreferences](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpTransceiver/setCodecPreferences)
on MDN.
...
and [Using WebRTC Encoded Transforms](https://developer.mozilla.org/en-US/docs/Web/API/WebRTC_API/Using_Encoded_Transforms)
on MDN.
