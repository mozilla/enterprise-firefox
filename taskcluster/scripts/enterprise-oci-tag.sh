...
#
# Ship a Firefox Enterprise OCI release by tagging the digest that the OCI
# upload task staged. No blobs are re-uploaded; this is a manifest-only retag, so it ships
# exactly the image that was staged and verified.
#
# Consumes the `release-index.json` handoff produced by the upstream
# `enterprise-oci-upload` task and the prebuilt `moa` binary, both in
...
# Fetch the content to produce the .env file that `moa` reads via dotenv. The
# secret stores it ready to use; the only work left is unwrapping the
# {"secret": ..., "expires": ...} envelope the secrets API responds with.
...
handoff=$(
    python3 -c 'import json,sys; j=json.load(sys.stdin); print(j["index"], j["channel"])' < "${index_file}"
)
read -r index channel <<< "${handoff}"

"${moa}" tag release "${index}" "${channel}"
