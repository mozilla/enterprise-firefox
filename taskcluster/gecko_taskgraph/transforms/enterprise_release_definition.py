...
def _build_policies(raw_base, revision, build_assets):
    ...
    changesets = _load_changesets(os.path.join(GECKO, build_assets["enterprise-l10n"]))
...
@functools.cache
def _load_changesets(path):
    """The l10n changesets at ``path``, or ``{}`` if the file does not exist.

    Cached because every release definition task reads the same file, which
    does not change while the graph is generated. Callers must not mutate the
    result.
    """
    try:
