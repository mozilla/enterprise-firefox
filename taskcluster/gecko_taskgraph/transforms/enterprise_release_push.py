...
The partner repack identity (``partner``/``sub_config``) reaches these tasks as
attributes, inherited with ``copy-attributes`` from the primary ``from_deps``
dependency -- the release-definition task for push, the push task for ship --
and is what the registry reference is derived from. Each task also gets a
unique treeherder symbol and description; the phase verb (``push``/``ship``)
comes from the kind name.
...
def set_repack_metadata(config, tasks):
    ...
    for task in tasks:
        # `copy-attributes` carried these over from the release definition
        # task, and carries them on to ship from here.
