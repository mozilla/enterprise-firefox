...
def get_repack_configs_by_platform(config, build_platform):
    """The repack configs of ``build_platform``, mapped to their locales.

    A repack config is ``{partner}/{sub_config}``. It is what the enterprise
    repack chain fans out on, one step coarser than the
    ``{partner}/{sub_config}/{locale}`` repack ids the partner and eme-free
    kinds use.
    """
    partner_config = get_partner_config_by_kind(config, config.kind)
    configs = {}
    for partner, subconfigs in partner_config.items():
        for sub_config_name, sub_config in subconfigs.items():
            ...
            if not locales:
                continue
            configs[f"{partner}/{sub_config_name}"] = sorted(locales)
