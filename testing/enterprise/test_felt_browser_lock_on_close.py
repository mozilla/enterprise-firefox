...
PREF_CONNECTION_LOSS_ACTION = "enterprise.connection_loss.action"
...
class BrowserLockOnClose(FeltTests):
    ...
    def _begin_connection_loss_test(self, action):
        """Sign in and set the ConnectionLoss action, returning the child pid."""
        browser_pid = self._start_signed_in(keep_felt_window=False)
        # The ConnectionLoss prefs ship locked, and set_prefs can't modify a
        # locked pref; unlock it so the set_prefs below takes effect.
        with self._child_driver.using_context("chrome"):
            self._child_driver.execute_script(
                "Services.prefs.unlockPref(arguments[0]);",
                script_args=[PREF_CONNECTION_LOSS_ACTION],
            )
        self._child_driver.set_prefs({PREF_CONNECTION_LOSS_ACTION: action})
        assert self.signout_count.value == 0, "No signout should have been posted yet"
        return browser_pid

    def _end_session_for_connection_loss(self):
        ...
        self._child_driver.execute_script(
            ...
            Services.tm.dispatchToMainThread(() => {
                EnterpriseHandler.endSessionForConnectionLoss();
            });
            """
        )
    ...
    def test_connection_loss_lock_explains_itself(self):
        """ConnectionLoss=lock: the session locks and FELT says why.
        ...
        """
        browser_pid = self._begin_connection_loss_test("lock")

        self._end_session_for_connection_loss()
        self._settle_after_close(browser_pid)
    ...
    def test_connection_loss_signout_explains_itself(self):
        """ConnectionLoss=signout: the session ends and FELT says why."""
        browser_pid = self._begin_connection_loss_test("signout")
